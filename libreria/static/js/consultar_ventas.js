document.addEventListener('DOMContentLoaded', function() {
    ...
    searchSelect.addEventListener('change', function() {
        ...
        if (selectedOption === 'todos') {
            ...
            searchDate.value = ''; // Limpiar el valor del input de fecha
        } else if (selectedOption === 'fecha') {
            searchInput.style.display = 'none';
            searchDate.style.display = 'inline-block'; // Mostrar el input de fecha
            searchInput.value = ''; // Limpiar el valor del input de texto
        } else {
            searchInput.style.display = 'inline-block'; // Asegúrate de que se muestre el input
            searchDate.style.display = 'none'; // Ocultar el input de fecha
            searchInput.value = ''; // Limpiar el valor del input de texto
        }
    });
    ...
    function performSearch() {
        ...
        var searchDateValue = searchDate.value; // Obtener el valor de la fecha
        var rows = tableBody.getElementsByTagName('tr');
        var found = false;

        // Mostrar aviso si el campo está vacío y no es "todos"
        if ((searchText === '' && selectedOption !== 'todos' && selectedOption !== 'fecha') ||
            (selectedOption === 'fecha' && searchDateValue === '')) {
        ...
        Array.from(rows).forEach(function(row) {
            ...
            } else if (selectedOption === 'tipo_transaccion') {
                shouldDisplay = cells[0].textContent.toLowerCase().includes(searchText); // Filtrar por tipo de transacción
            } else if (selectedOption === 'cliente') {
                shouldDisplay = cells[1].textContent.toLowerCase().includes(searchText); // Filtrar por cliente
            } else if (selectedOption === 'descripcion') {
                shouldDisplay = cells[2].textContent.toLowerCase().includes(searchText); // Filtrar por descripción
            } else if (selectedOption === 'monto') {
                shouldDisplay = cells[3].textContent.toLowerCase().includes(searchText); // Filtrar por monto
            } else if (selectedOption === 'fecha') {
                // Obtener la fecha de la celda y convertirla al formato YYYY-MM-DD
                var dateCell = cells[4].textContent.trim(); // Fecha en formato d/m/Y
                var parts = dateCell.split('/'); // Divide en partes
                var formattedDate = parts[2] + '-' + parts[1] + '-' + parts[0]; // Convierte a YYYY-MM-DD

                shouldDisplay = (formattedDate === searchDateValue); // Filtrar por fecha
            }
...
async function exportToExcel() {
    ...
    try {
        ...
        worksheet.mergeCells('A1:D1'); // Combina cuatro columnas (A, B, C, D)
    ...
    workbook.xlsx.writeBuffer().then(buffer => {
        saveAs(new Blob([buffer]), 'Tabla_Ventas.xlsx');
    }).catch(error => {
