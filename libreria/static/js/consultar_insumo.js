    ...
    function performSearch() {
        ...
        Array.from(rows).forEach(function(row) {
            ...
            if (selectedOption === 'todos') {
                ...
                shouldDisplay = cells[3].textContent.toLowerCase().includes(searchText); // Filtrar por proveedor
            } else if (selectedOption === 'cantidad') {
                shouldDisplay = cells[4].textContent.toLowerCase().includes(searchText); // Filtrar por cantidad
            }
...
async function exportToExcel() {
    ...
    workbook.xlsx.writeBuffer().then(buffer => {
        saveAs(new Blob([buffer]), 'Tabla_Insumos.xlsx');
    }).catch(error => {
