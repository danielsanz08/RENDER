document.addEventListener('DOMContentLoaded', function() {
    ...
    function performSearch() {
        ...
        Array.from(rows).forEach(function(row) {
            ...
            } else if (selectedOption === 'nit') {
                shouldDisplay = cells[1].textContent.toLowerCase().includes(searchText); // Filtrar por NIT
            } else if (selectedOption === 'nombre') {
                shouldDisplay = cells[2].textContent.toLowerCase().includes(searchText); // Filtrar por nombre
            } else if (selectedOption === 'direccion') {
                shouldDisplay = cells[3].textContent.toLowerCase().includes(searchText); // Filtrar por dirección
            } else if (selectedOption === 'email') {
                shouldDisplay = cells[4].textContent.toLowerCase().includes(searchText); // Filtrar por email
            } else if (selectedOption === 'telefono') {
                shouldDisplay = cells[5].textContent.toLowerCase().includes(searchText); // Filtrar por teléfono
            }
...
function exportToPDF() {
    ...
    watermark.onload = () => {
        ...
        html2pdf().from(element).set({
            ...
            pdf.save('Tabla_Proveedores.pdf');
        }).catch(error => {
...
async function exportToExcel() {
    ...
    try {
        ...
        worksheet.mergeCells('A1:F1'); // Combina cuatro columnas (A, B, C, D)
    ...
    workbook.xlsx.writeBuffer().then(buffer => {
        saveAs(new Blob([buffer]), 'Tabla_Proveedores.xlsx');
    }).catch(error => {
