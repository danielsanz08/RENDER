    ...
    function performSearch() {
        ...
        Array.from(rows).forEach(function(row) {
            ...
            if (selectedOption === 'todos') {
                ...
                shouldDisplay = cells[0].textContent.toLowerCase().includes(searchText); // Filtrar por nombre
            } else if (selectedOption === 'presentacion') {
                shouldDisplay = cells[1].textContent.toLowerCase().includes(searchText); // Filtrar por presentación
            } else if (selectedOption === 'medida') {
                shouldDisplay = cells[2].textContent.toLowerCase().includes(searchText); // Filtrar por unidad de medida
            } else if (selectedOption === 'cantidad') {
                shouldDisplay = cells[3].textContent.toLowerCase().includes(searchText); // Filtrar por cantidad
            } else if (selectedOption === 'precio') {
                shouldDisplay = cells[4].textContent.toLowerCase().includes(searchText); // Filtrar por precio
            }
...
async function exportToExcel() {
    ...
    workbook.xlsx.writeBuffer().then(buffer => {
        saveAs(new Blob([buffer]), 'Tabla_Productos.xlsx');
    }).catch(error => {
