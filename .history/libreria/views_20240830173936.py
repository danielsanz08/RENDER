...
from .forms import InsumoForm, TransaccionForm, CustomUserCreationForm, LoginForm, CustomUserChangeForm,CustomUserForm
...
@login_required
def inactivar_usuarios(request):
    if request.method == 'POST':
        form = CustomUserForm(request.POST, request.FILES, instance=request.user)
        if form.is_valid():
            form.save()
            return redirect('profile')  # Redirige a la página de perfil o a donde necesites
    else:
