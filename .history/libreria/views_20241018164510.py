...
def crear_transacciones(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            print(form.cleaned_data)  # Check what data is being processed
            
            nueva_transaccion = form.save(commit=False)

            # Asignar el cliente y producto desde el formulario
            nueva_transaccion.cliente = form.cleaned_data['cliente']  # Debe ser una instancia válida de Cliente
            
            # Asegúrate de que producto sea un ForeignKey válido
            producto_id = form.cleaned_data['producto']  # Esto debe ser el ID del producto
            nueva_transaccion.producto = get_object_or_404(Producto, pk=producto_id)  # Asegúrate de que esto sea una instancia válida
            
            nueva_transaccion.registrado_por = usuario  # Asignar el usuario que creó la transacción
            nueva_transaccion.save()
            messages.success(request, 'Transacción creada exitosamente.')
            return redirect('ver_transacciones')  # Cambia esto al nombre de la vista adecuada
        else:
            messages.error(request, 'Por favor corrige los errores a continuación.')
...
def home_view(request):
    ...
    hay_administrador = CustomUser.objects.filter(role='Administrador').exists()

    # Redirigir según la verificación
    if not hay_administrador:
        return redirect('crear_perfil')  # Redirige al registro si no hay administrador

    # Si hay un administrador, puedes redirigir a otra página o mostrar un mensaje
    return redirect('password_reset')  # Redirige al restablecimiento de contraseña
@login_required
