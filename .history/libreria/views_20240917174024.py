...
@login_required
def verificar_contraseña_admin(request):
    if request.method == 'POST':
        admin_password = request.POST.get('admin_password')
        user = get_object_or_404(User, role='Administrador')

        if check_password(admin_password, user.password):
            # La contraseña es correcta, redirigir a la página de creación de perfil
            return redirect('crear_perfil')
        else:
            # La contraseña es incorrecta
            messages.error(request, 'La contraseña del administrador es incorrecta.')
            return render(request, 'usuario/ver.html', {'messages': messages.get_messages(request)})

    # Redirigir a una página segura si el método no es POST
