...
from .models import Transaccion, Insumo, CustomUser, Backup,Cliente,Producto
from django.contrib.auth.decorators import login_required
from .forms import InsumoForm, TransaccionForm, CustomUserCreationForm, LoginForm, CustomUserChangeForm,ClienteForm,ProductoForm
...
def crear_perfil(request):
    if request.method == 'POST':
        ...
        if hasattr(request.user, 'role') and request.user.role == 'Empleado':
            ...
            # If the user is an administrator, show the form directly
            form = CustomUserCreationForm(request.POST, request.FILES)
            if form.is_valid():
                user = form.save()
                messages.success(request, f'Usuario {user.email} creado exitosamente.')
    ...
    return render(request, 'usuario/registro.html', {'form': form, 'breadcrumbs': breadcrumbs, 'user_role': user_role})
@login_required
...
def productos(request):
    ...
    breadcrumbs = [{'name': 'Inicio', 'url': '/'}, {'name': 'Gestión de Productos', 'url': '/productos/'}]
    return render(request, 'productos/producto.html', {'productos': productos, 'breadcrumbs': breadcrumbs})
...
def crear_producto(request):
    if request.method == 'POST':
        ...
        if producto_form.is_valid():
            ...
            return redirect('lista_productos')  # Cambia a tu URL de lista de productos
    else:
...
def editar_producto(request, pk):
    ...
    breadcrumbs = [{'name': 'Inicio', 'url': '/'}, {'name': 'Gestión de Productos', 'url': '/productos/'}, {'name': 'Editar Producto', 'url': '/editar_producto/'}]
    return render(request, 'productos/editar_producto.html', {'form': form, 'producto': producto, 'breadcrumbs': breadcrumbs})
...
def lista_productos(request):
    # Lógica para manejar la solicitud y devolver una respuesta
...
def listar_proveedor(request):
    return render(request, 'proveedores/listar_proveedor.html')