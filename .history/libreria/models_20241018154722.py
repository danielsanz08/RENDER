...
class Producto(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.lote or self.lote == 'lote_default':
            self.lote = f'lote_{uuid4().hex}'
        super().save(*args, **kwargs)
...
class Transaccion(models.Model):
    ...
    monto = models.DecimalField(max_digits=15, decimal_places=2)  # Para manejar precios
    fecha = models.DateField(auto_now_add=True)  # Fecha de creación de la transacción
    registrado_por = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True)  # Clave foránea a CustomUser
    cliente = models.ForeignKey(Cliente, on_delete=models.SET_NULL, null=True, blank=True)  # Clave foránea a Cliente

    def __str__(self):
        return f"{self.get_tipo_display()} - {self.descripcion or 'Sin descripción'} - {self.monto}"

    class Meta:
        verbose_name = "Transacción"
        verbose_name_plural = "Transacciones"
        ordering = ['-fecha']  # Ordenar las transacciones por fecha, las más recientes primero