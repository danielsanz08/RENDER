...
def ver_perfil(request):
    ...
    if usuario.role == 'Administrador':
        # Si el usuario es Administrador, mostrar directamente la página del perfil
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                if check_password(admin_password, admin_user.password):
                    # Si la contraseña es correcta, renderizamos la página del perfil
                    return render(request, 'usuario/ver.html', {'usuario': usuario, 'breadcrumbs': breadcrumbs})
                else:
        ...
        # Si es GET, mostramos el formulario para ingresar la contraseña
        return render(request, 'usuario/verificar_admin.html', {'usuario': usuario})

    else:
        # Para otros roles, también mostramos directamente la página del perfil
        return render(request, 'usuario/ver.html', {'usuario': usuario, 'breadcrumbs': breadcrumbs})
...
@csrf_exempt
def verify_admin_password_js(request):
    if request.method == 'POST':
        import json
