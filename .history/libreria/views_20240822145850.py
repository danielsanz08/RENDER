...
from .models import Transaccion, Insumo,CustomUser
...
def listar_usuario(request):
    ...
    return render(request, 'usuario/listar.html', {'transacciones': tr})

@login_required
def eliminar_perfil(request, id):
    try:
        user = CustomUser.objects.get(id=id)  # Asegúrate de usar el modelo correcto para usuarios
        user.delete()
        messages.success(request, 'El perfil ha sido eliminado exitosamente.')
        return redirect('listar_usuario')  # Redirige a una página de listado de usuarios o donde prefieras
    except CustomUser.DoesNotExist:
        messages.error(request, 'El perfil que intentas eliminar no existe.')
