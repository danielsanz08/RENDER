...
def editar_insumo(request, pk):
    ...
    if request.method == 'POST':
        ...
        # Solo validamos y guardamos el campo 'nombre'
        if form.is_valid():
            ...
            if Insumo.objects.filter(nombre=nombre_nuevo).exclude(pk=insumo.pk).exists():
                ...
                insumo.nombre = nombre_nuevo  # Solo actualiza el nombre
                insumo.save()  # Guarda los cambios
    ...
    breadcrumbs = [
        ...
        {'name': 'Editar Insumo', 'url': request.path},  # Usa request.path para obtener la URL actual
    ]
