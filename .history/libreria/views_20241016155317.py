...
def editar_insumo(request, insumo_id):
    usuario = request.user  # Obtiene el usuario actual que ha iniciado sesión
    try:
        insumo = Insumo.objects.get(id=insumo_id)  # Obtiene el insumo a editar
    except Insumo.DoesNotExist:
        messages.error(request, 'Insumo no encontrado.')
        return redirect('consultar_insumo')

    if request.method == 'POST':
        form = InsumoForm(request.POST, instance=insumo)  # Prepara el formulario con los datos del insumo existente
        if form.is_valid():
            proveedor_id = request.POST.get('proveedor')  # Captura el proveedor seleccionado
            try:
                proveedor = Proveedor.objects.get(id=proveedor_id)  # Asegúrate de que existe el proveedor
                insumo = form.save(commit=False)  # Guarda los cambios
                insumo.proveedor = proveedor  # Asocia el proveedor al insumo
                insumo.registrado_por = request.user  # Asigna el usuario que registra
                insumo.save()
        ...
        form = InsumoForm(instance=insumo)  # Carga el formulario con los datos existentes
    ...
    breadcrumbs = [
        {'name': 'Inicio', 'url': '/inicio'},
        {'name': 'Gestión de Proveedores', 'url': '/insumos'},
        {'name': 'Editar insumo', 'url': f'/editar_insumo/{insumo_id}'},
    ]
    ...
    return render(request, 'insumos/editar_insumo.html', {
        ...
        'proveedores': proveedores,
        'breadcrumbs': breadcrumbs,
        'insumo': insumo,  # Pasar el insumo actual a la plantilla
    })
