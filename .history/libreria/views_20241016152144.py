...
def editar_insumo(request, pk):
    usuario = request.user  # Obtiene el usuario actual que ha iniciado sesión
    insumo = get_object_or_404(Insumo, pk=pk)  # Obtiene el insumo a editar

    if request.method == 'POST':
        form = InsumoForm(request.POST, instance=insumo)
        original_nombre = request.POST.get('original-nombre')  # Obtiene el nombre original

        if form.is_valid():
            # Verifica si se está editando el nombre
            nombre_nuevo = form.cleaned_data['nombre']
            if nombre_nuevo != original_nombre:  # Solo verifica duplicados si el nombre ha cambiado
                if Insumo.objects.filter(nombre=nombre_nuevo).exclude(pk=insumo.pk).exists():
                    form.add_error('nombre', 'Ya existe un insumo con ese nombre.')
                else:
                    form.save()  # Guarda todos los cambios del formulario
                    messages.success(request, 'Insumo actualizado exitosamente.')
                    return redirect('consultar_insumo')  # Redirige a la lista de insumos
            else:
                form.save()  # Guarda si el nombre no ha cambiado

        # En caso de error, puedes imprimirlo para depurar
        print(form.errors)  # Imprime errores de validación

    else:
        form = InsumoForm(instance=insumo)  # Carga el formulario con el insumo existente
    ...
    breadcrumbs = [
        ...
        {'name': 'Editar Insumo', 'url': request.path},
    ]
