...
def editar_perfil(request):
    ...
    return render(request, 'usuario/editar.html', {'usuario': usuario, 'form': form, 'breadcrumbs': breadcrumbs})
...
def backup_view(request):
    if request.method == 'POST':
        if 'create_backup' in request.POST:
            ...
            if backup_id and restore_backup(backup_id):
                messages.success(request, 'Base de datos restaurada exitosamente.')
            else:
                messages.error(request, 'Error al restaurar la base de datos. Asegúrate de que el ID de la copia de seguridad sea válido.')

        elif 'delete_backup' in request.POST:
            backup_id = request.POST.get('backup_id')
            if backup_id:  # Verificar que el ID de la copia de seguridad no esté vacío
                backup = get_object_or_404(Backup, id=backup_id)
                backup.delete()
                messages.success(request, 'Copia de seguridad eliminada exitosamente.')
            else:
                messages.error(request, 'ID de copia de seguridad no válido.')

    # Obtener todas las copias de seguridad y ordenarlas por fecha de creación
    backups = Backup.objects.all().order_by('-created_at')

    # Define las migas de pan para la navegación
    breadcrumbs = [
        {'name': 'Inicio', 'url': '/'},
...
def crear_cliente(request):
    ...
    if request.method == 'POST':
        ...
        if cliente_form.is_valid():  # Verifica si el formulario es válido
            cliente_form.save()  # Guarda el nuevo cliente en la base de datos
            return redirect('consultar_clientes')  # Cambia a la URL correcta
    else:
    ...
    breadcrumbs = [
        {'name': 'Inicio', 'url': '/inicio/'},
        {'name': 'Clientes', 'url': '/clientes/'},
        {'name': 'Agregar Cliente', 'url': '/'}
    ]

    return render(request, 'clientes/crear_cliente.html', {'usuario': usuario, 'cliente_form': cliente_form, 'breadcrumbs': breadcrumbs})  # Asegúrate de pasar el formulario al template
