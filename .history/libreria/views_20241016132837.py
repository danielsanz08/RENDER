...
from .models import Transaccion, Insumo, CustomUser, Backup,Cliente,Producto,Proveedor
...
def ver_transacciones(request):
    ...
    return render(request, 'movimientos/ver_movimientos.html', {'usuario': usuario, 'transacciones': transacciones, 'breadcrumbs': breadcrumbs})
...
def editar_insumo(request, pk):
    ...
    return render(request, 'insumos/editar_insumo.html', {'usuario': usuario, 'form': form, 'breadcrumbs': breadcrumbs})
...
def crear_cliente(request):
    if request.method == 'POST':
        ...
        if cliente_form.is_valid():  # Verifica si el formulario es válido
            cliente = cliente_form.save(commit=False)  # Crea una instancia del modelo sin guardar en la BD aún
            cliente.registrado_por = request.user  # Asigna el usuario que está creando el cliente

            try:
                cliente.save()  # Intenta guardar el cliente en la base de datos
            ...
            except Exception as e:  # Captura cualquier excepción durante el guardado
                messages.error(request, f'Error al crear el cliente: {str(e)}')  # Mensaje de error
                print(f"Error al guardar el cliente: {e}")  # Imprime el error en la consola para depuración
        else:
            messages.error(request, 'Por favor, corrige los errores en el formulario.')  # Mensaje de error si el formulario no es válido
            print(cliente_form.errors)  # Imprime los errores del formulario en la consola para depuración
    else:
        cliente_form = ClienteForm()  # Si no es POST, crea un formulario vacío

    # Datos de breadcrumbs para la navegación
    breadcrumbs = [
        {'name': 'Inicio', 'url': '/inicio/'},
        {'name': 'Clientes', 'url': '/clientes/'},
        {'name': 'Agregar Cliente', 'url': '/clientes/crear/'}  # Asegúrate de que esta URL sea correcta
    ]

    # Renderiza la plantilla con los datos del formulario y los breadcrumbs
    return render(request, 'clientes/crear_cliente.html', {
        'usuario': request.user,  # Proporciona el usuario actual
        'cliente_form': cliente_form,  # Pasa el formulario al template
        'breadcrumbs': breadcrumbs  # Pasa los breadcrumbs para la navegación
    })
@login_required
...
def editar_producto(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('lista_productos')  # Asegúrate de que esta URL esté configurada correctamente
        else:
            print(form.errors)  # Esto te dará información sobre qué está fallando 
    else:
        form = ProductoForm(instance=producto)
    
    # Crear el contexto con el formulario, el producto, y las migas de pan
    breadcrumbs = [
        {'name': 'Inicio', 'url': '/'},
        {'name': 'Gestión de Productos', 'url': '/productos/'},
        {'name': 'Productos Registrados', 'url': '/lista_productos/'},
        {'name': 'Editar Producto', 'url': f'/editar_producto/{pk}/'}  # Asegúrate de incluir el ID del producto
    ]
