...
def verificar_contraseña_admin(request):
    if request.method == 'POST':
        admin_password = request.POST.get('admin_password')
        user = authenticate(request, username=request.user.username, password=admin_password)
        if user is not None:
    ...
    return redirect('index')  # Redirigir a una página segura si el método no es POST