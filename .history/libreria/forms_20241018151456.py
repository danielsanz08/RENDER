...
class TransaccionForm(forms.ModelForm):
    class Meta:
        model = Transaccion
        fields = ['cliente', 'tipo', 'monto', 'descripcion', 'fecha']
        widgets = {
            'fecha': forms.DateInput(attrs={'type': 'date'}),  # Añade un DatePicker al campo de fecha
            'descripcion': forms.Textarea(attrs={'rows': 3}),  # Ajusta el tamaño del textarea
        }
        labels = {
            'cliente': 'Cliente',
            'tipo': 'Tipo de Transacción',
            'monto': 'Monto (en la moneda correspondiente)',
            'descripcion': 'Descripción (opcional)',
            'fecha': 'Fecha de la Transacción',
        }
 # Removed duplicate 'cliente'
...
class ProveedorForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Modificar el campo tipo_persona
        self.fields['tipo_persona'].widget.attrs.update({'class': 'form-control'})
        self.fields['tipo_persona'].empty_label = "Seleccione tipo de persona"

        # Asegurarse de que la opción vacía esté al principio de las opciones
