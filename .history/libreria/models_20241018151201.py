...
class Producto(models.Model):
    ...
    codigo = models.CharField(max_length=100, unique=True)  # Asegúrate que el código sea único
    nombre = models.CharField(max_length=100)
    ...
    def clean(self):
        # Validar que la fecha de vencimiento sea posterior a la fecha de elaboración
    ...
    def save(self, *args, **kwargs):
        if not self.lote or self.lote == 'lote_default':
            self.lote = f'lote_{uuid4().hex[:8]}'  # Genera un lote único si no se proporciona uno
        self.clean()
        super().save(*args, **kwargs)
...
class Transaccion(models.Model):
    ...
    producto = models.ForeignKey(Producto, on_delete=models.CASCADE)  # Asegúrate de que el ID del producto sea correcto
    descripcion = models.CharField(max_length=100, verbose_name='Descripción')
    monto = models.DecimalField(max_digits=10, decimal_places=3, default=100.00, verbose_name="Monto")
    fecha = models.DateField()

    def __str__(self):
        return f"{self.tipo} - {self.descripcion} - ${self.monto} - Producto: {self.producto.nombre}"
